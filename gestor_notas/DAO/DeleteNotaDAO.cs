...
{
    public class DeleteNotaDAO : IDeleteNotaDAO
    {
        public async Task<bool> DeleteNotaAsync(NpgsqlConnection connection, int idNota, int idProfesor)
        {
            try
            {
                var query = "DELETE FROM nota WHERE id = @idNota AND id_materia IN (SELECT id FROM materia WHERE id_profesor = @idProfesor);";
